...
#ifndef SIGNAL_SIZE
#define SIGNAL_SIZE        _N_
#endif

#ifndef _BATCH_SIZE_
#define _BATCH_SIZE_       (128)
#endif
...
void runTest(int argc, char **argv)
{
    ...
    Complex *d_signal;
    checkCudaErrors(cudaMalloc((void **)&d_signal, mem_size * _BATCH_SIZE_));
    // Copy host memory to device
    checkCudaErrors(cudaMemcpy(d_signal, h_padded_signal, mem_size * _BATCH_SIZE_,
    ...
    checkCudaErrors(cufftXtMakePlanMany(plan_adv, 1, &new_size_long, NULL, 1, 1, CUDA_C_32F, NULL,
          1, 1, CUDA_C_32F, _BATCH_SIZE_, &workSize, CUDA_C_32F));
    printf("Temporary buffer size %li bytes\n", workSize);

    // Transform signal and kernel
    printf("Transforming signal cufftExecC2C\n");
	// Calculate the Signal FFT: Complex-to-Complex
    printf("Calculate the simple FFT time, batch = %d, single length = %d, Tick Time = ", _BATCH_SIZE_, SIGNAL_SIZE);
    ...
    checkCudaErrors(cudaDeviceSynchronize());
    end_roi();
	  //// Calculate the Filter Kernal Signal
    ...
    //checkCudaErrors(cudaMemcpy(h_convolved_signal, d_signal, mem_size * _BATCH_SIZE_,
