...
int main(int argc, char* argv[]) {
  ...
  SB_CONST(P_add_in2,0,1);
  SB_DMA_READ_STRETCH(&input[0],8,AWORDS*8,-8,AWORDS,P_add_in1);
  SB_RECURRENCE(P_add_out,P_add_in2,iters-1);
  SB_DMA_WRITE(P_add_out,8,8,1,&my_answer);
  SB_WAIT_ALL();
  end_roi();
