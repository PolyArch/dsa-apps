...
void mv(long tid) {
  ...
  int start_col = tid*(V/C);
  int end_col = (tid+1)*(V/C);
  
  // read activations -- move to linear scratchpad later
  // FIXME: Why couldn't it pop rest 10 elements? -- 27 is reached already...
  SS_VREPEAT_PORT(P_pr_row_size2);
  // 41 -- that means correct... (in some earlier accesses, it was not
  //    considered that means that setting to -1 didn't work)
...
// offset list should be read directly
/*
SS_REPEAT_PORT(1);
SS_CONST(P_pr_offset_list,offset[0],1);
SS_REPEAT_PORT(2); // have i broken this?
SS_DMA_READ(&offset[1], 2, 2, V-1, P_pr_offset_list);
SS_REPEAT_PORT(1);
SS_CONST(P_pr_offset_list,offset[V],1);
*/
