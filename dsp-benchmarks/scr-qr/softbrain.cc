...
// Finalize the result
    {
      ...
      SB_CONST(P_finalize_W, *((uint64_t*)&w), N * len);
      SB_DMA_READ(qx, 8, 8, N * len, P_finalize_Q);
      SB_SCR_PORT_STREAM(1, 0, 8 * len, N, P_finalize_VQ);
      ...
      SB_REPEAT_PORT(len);
      SB_SCRATCH_READ(tmp0, 8 * N, P_finalize_TMPQ);

      SB_2D_CONST(P_finalize_MUX0, 0, 1, 1, len - 1, N);
      SB_DMA_WRITE(P_finalize_O00, 8 * N, 8, N, Q + i);
      SB_DMA_WRITE(P_finalize_O01, 8, 8, (len - 1) * N, nxt);

      {
        SB_SCRATCH_READ(tmp1, 8 * len, P_finalize_TMPR);
        //SB_DMA_READ(r, len * 8, 8, len, P_finalize_R);
        SB_SCR_PORT_STREAM(r, len * 8, 8, len, P_finalize_R);
        SB_SCR_PORT_STREAM(0, 0, 8, len, P_finalize_VR);
      ...
      SB_SCR_PORT_STREAM(r + (len + 1) * 8, 8 * len, 8 * (len - 1), len - 1, P_finalize_R);
      SB_SCR_PORT_STREAM(8, 0, 8 * (len - 1), len - 1, P_finalize_VR);
      //SB_DMA_WRITE(P_finalize_O1, 8, 8, (len - 1) * (len - 1), r);
      SB_SCR_WRITE(P_finalize_O1, 8 * (len - 1) * (len - 1), r);


      SB_REPEAT_PORT(len - 1);
      SB_SCRATCH_READ(tmp1 + 8, 8 * (len - 1), P_finalize_TMPR);
