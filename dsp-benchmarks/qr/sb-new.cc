...
void qr(complex<float> *a, complex<float> *q, complex<float> *tau) {
  ...
  for (int i = 0; i < N - 1; ++i) {
    ...
    SB_SCRATCH_READ(r_trans_spad + 8, 8 * (n - 1), P_rdfg_M);
    SB_SCRATCH_READ(r_trans_spad + 8, 8 * (n - 1), P_rdfg_V);
    ...
    SB_STRIDE(8, 8);
    SB_REPEAT_PORT(n - 1);
    SB_RECURRENCE(P_rdfg_U1INV, P_rdfg_M, 1); //normalize
    SB_DMA_WRITE(P_rdfg_TAU0, 8, 8, 1, tau + i);
    SB_CONST(P_rdfg_Coef, _one_in_bit, n - 1);
    SB_SCR_PORT_STREAM(r_trans_spad + 8, 8, 8, n - 1, P_rdfg_V);
    ...
    SB_SCR_PORT_STREAM(w_spad, 0, 8 * n, n - 1, P_rdfg_M);
    SB_SCR_PORT_STREAM(r_trans_spad, 8 * n, 8 * n, n - 1, P_rdfg_V);
    ...
    SB_SCR_PORT_STREAM(w_spad, 0, 8 * n, n - 1, P_rdfg_B);
    SB_SCR_PORT_STREAM(r_trans_spad, 8 * n, 8 * n, n - 1, P_rdfg_C);
    if (n - 1 == 1) {
      SB_DMA_WRITE(P_rdfg_R, 0, 8, 1, a + N * N - 1);
    } else {
      SB_SCR_WRITE(P_rdfg_R, 8 * (n - 1) * (n - 1), r_trans_spad);
    }
    SB_DMA_WRITE(P_rdfg_FIN, 8, 8, n - 1, a + i * N + i + 1);
    SB_GARBAGE(P_rdfg_R, (n - 1) * (n % 2));
  ...
  for (int i = N - 2; i >= 0; --i) {
    ...
    SB_FILL_MODE(STRIDE_ZERO_FILL);
    SB_SCR_PORT_STREAM(w_spad + 8, 0, 8 * (n - 1), n - 1, P_qdfg_M);
    SB_DMA_READ(q + (i + 1) * N + i + 1, 8 * N, 8 * (n - 1), n - 1, P_qdfg_V);
    SB_CONST(P_qdfg_Coef, *((uint64_t*)(tau + i)), n12 * (n - 1));
    SB_2D_CONST(P_qdfg_reset, 2, n12 - 1, 1, 1, n - 1);
    //SB_GARBAGE(P_qdfg_O, n - 1);

    SB_FILL_MODE(STRIDE_DISCARD_FILL);
    SB_REPEAT_PORT(n2);
    SB_RECURRENCE(P_qdfg_O, P_qdfg_A, n - 1);
    SB_SCR_PORT_STREAM(w_spad, 0, 8 * n, n - 1, P_qdfg_B);
    SB_DMA_READ(q + (i + 1) * N + i, 8 * N, 8 * n, n - 1, P_qdfg_C);
    SB_DMA_WRITE(P_qdfg_Q, 8 * N, 8 * n, n - 1, q + (i + 1) * N + i);
    //SB_GARBAGE(P_qdfg_Q, (n - 1) * (n % 2));
