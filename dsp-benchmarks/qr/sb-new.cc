...
void qr(complex<float> *a, complex<float> *q, complex<float> *tau) {
  ...
  for (int i = 0; i < N - 1; ++i) {
    ...
    SS_SCRATCH_READ(r_trans_spad + 8, 8 * (n - 1), P_rdfg_M);
    SS_SCRATCH_READ(r_trans_spad + 8, 8 * (n - 1), P_rdfg_V);
    ...
    SS_STRIDE(8, 8);
    SS_REPEAT_PORT(n - 1);
    SS_RECURRENCE(P_rdfg_U1INV, P_rdfg_M, 1); //normalize
    SS_DMA_WRITE(P_rdfg_TAU0, 8, 8, 1, tau + i);
    SS_CONST(P_rdfg_Coef, _one_in_bit, n - 1);
    SS_SCR_PORT_STREAM(r_trans_spad + 8, 8, 8, n - 1, P_rdfg_V);
    ...
    SS_SCR_PORT_STREAM(w_spad, 0, 8 * n, n - 1, P_rdfg_M);
    SS_SCR_PORT_STREAM(r_trans_spad, 8 * n, 8 * n, n - 1, P_rdfg_V);
    ...
    SS_SCR_PORT_STREAM(w_spad, 0, 8 * n, n - 1, P_rdfg_B);
    SS_SCR_PORT_STREAM(r_trans_spad, 8 * n, 8 * n, n - 1, P_rdfg_C);
    if (n - 1 == 1) {
      SS_DMA_WRITE(P_rdfg_R, 0, 8, 1, a + N * N - 1);
    } else {
      SS_SCR_WRITE(P_rdfg_R, 8 * (n - 1) * (n - 1), r_trans_spad);
    }
    SS_DMA_WRITE(P_rdfg_FIN, 8, 8, n - 1, a + i * N + i + 1);
    SS_GARBAGE(P_rdfg_R, (n - 1) * (n % 2));
  ...
  for (int i = N - 2; i >= 0; --i) {
    ...
    SS_FILL_MODE(STRIDE_ZERO_FILL);
    SS_SCR_PORT_STREAM(w_spad + 8, 0, 8 * (n - 1), n - 1, P_qdfg_M);
    SS_DMA_READ(q + (i + 1) * N + i + 1, 8 * N, 8 * (n - 1), n - 1, P_qdfg_V);
    SS_CONST(P_qdfg_Coef, *((uint64_t*)(tau + i)), n12 * (n - 1));
    SS_2D_CONST(P_qdfg_reset, 2, n12 - 1, 1, 1, n - 1);
    //SS_GARBAGE(P_qdfg_O, n - 1);

    SS_FILL_MODE(STRIDE_DISCARD_FILL);
    SS_REPEAT_PORT(n2);
    SS_RECURRENCE(P_qdfg_O, P_qdfg_A, n - 1);
    SS_SCR_PORT_STREAM(w_spad, 0, 8 * n, n - 1, P_qdfg_B);
    SS_DMA_READ(q + (i + 1) * N + i, 8 * N, 8 * n, n - 1, P_qdfg_C);
    SS_DMA_WRITE(P_qdfg_Q, 8 * N, 8 * n, n - 1, q + (i + 1) * N + i);
    //SS_GARBAGE(P_qdfg_Q, (n - 1) * (n % 2));
