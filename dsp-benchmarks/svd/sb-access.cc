...
void svd(complex<float> *a, complex<float> *u, float *s, complex<float> *v) {
  for (int i = 0; i < _N_ - 1; ++i) {
    ...
    if (i != _N_ - 2) {
      ...
      SB_2D_CONST(P_mvc_reset, 2, len - 1, 1, 1, len);
      SB_DMA_WRITE(P_mvc_O, 8, 8, len, temp);
      //for (int j = 0; j < len; ++j) {
      ...
      if (!i) {
        ...
        SB_2D_CONST(P_vvc_C, 1065353216, 1, 0, _N_ - 1, _N_ - 2);
        SB_CONST(P_vvc_C, 0, 1);

        SB_REPEAT_PORT(_N_ - 1);
        SB_DMA_READ(hv, 8, 8, _N_ - 1, P_vvc_A);
        //for (int j = 1; j < _N_; ++j) {
        ...
        SB_REPEAT_PORT(_N_ - 1);
        SB_DMA_READ(hv, 8, 8, _N_ - i - 1, P_vm_A);
  ...
  SB_DMA_WRITE(P_finalize_O, 0, 8 * _N_ * _N_, 1, u);
  for (int i = 0; i < _N_; ++i) {
