...
void svd(complex<float> *a, complex<float> *u, float *s, complex<float> *v) {
  ...
  for (int i = 0; i < N - 1; ++i) {
    ...
    SB_CONFIG(kernel1_config, kernel1_size);
    SB_CONST(P_kernel1_C, *((uint64_t*)&_zero), len - 1);
    SB_DMA_READ((i ? r : a) + 1, 8 * len, 8 * (len - 1), len, P_kernel1_B);
    SB_RECURRENCE(P_kernel1_O, P_kernel1_C, (len - 1) * (len - 1));
    for (int k = 0; k < len; ++k) {
      SB_CONST(P_kernel1_A, *((uint64_t*)(hv + k)), len - 1);
    }
    SB_DMA_WRITE(P_kernel1_O, 8, 8, len - 1, temp + 1);
    SB_WAIT_ALL();
    ...
    SB_CONFIG(kernel2_config, kernel2_size);
    SB_DMA_READ((i ? r : a) + 1, 8 * len, 8 * (len - 1), len, P_kernel2_C);
    SB_DMA_READ(temp + 1, 0, 8 * (len - 1), len, P_kernel2_A);
    SB_DMA_WRITE(P_kernel2_O, 8, 8, (len - 1) * len, r);
    for (int j = 0; j < len; ++j) {
      SB_CONST(P_kernel2_B, *((uint64_t*)(hv + j)), len - 1);
    }
    SB_WAIT_ALL();
