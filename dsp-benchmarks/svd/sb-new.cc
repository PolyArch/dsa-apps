...
void implicit_kernel(complex<float> *d, complex<float> *f, complex<float> *v, int n) {
  ...
  SS_CONST(P_aplygvs_mat, *((uint64_t*) &a), 1);
  SS_CONST(P_aplygvs_mat, *((uint64_t*) &b), 1);
  SS_CONST(P_aplygvs_mat, *((uint64_t*) (d + 0)), 1);
  SS_CONST(P_aplygvs_mat, *((uint64_t*) (f + 0)), 1);
  ...
  for (int i = 1; i < n - 1; ++i) {
    ...
    SS_RECURRENCE(P_aplygvs_mat_, P_aplygvs_mat, 4);

    SS_REPEAT_PORT(N);
    SS_RECURRENCE(P_aplygvs_rot, P_aplygvs_C, 1);
    SS_REPEAT_PORT(N);
    SS_RECURRENCE(P_aplygvs_rot, P_aplygvs_S, 1);
  ...
  //SS_GARBAGE(P_aplygvs_F_, 1);
  SS_GARBAGE(P_aplygvs_rot, 2);
  SS_DMA_WRITE(P_aplygvs_mat_, 8, 8, 1, f + n - 2);
  SS_GARBAGE(P_aplygvs_mat_, 1);
  SS_DMA_WRITE(P_aplygvs_mat_, 8, 8, 1, d + n - 1);
  SS_GARBAGE(P_aplygvs_mat_, 1);
