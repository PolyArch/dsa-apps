...
void cholesky(complex<float> *a, complex<float> *L) { int N = _N_;
  ...
  for (int i = 1, acc = 0, addr = 0; i < N; ++i) {
    ...
    SS_XFER_RIGHT(P_temporal_O, P_temporal_VAL, 1);
    SS_XFER_RIGHT(P_temporal_O, P_temporal_IN, padded);
    SS_XFER_RIGHT(P_temporal_O, P_temporal_Z, n * n / 2);

    acc = (acc + 1) & 7;
    SS_CONTEXT(1 << acc);

    SS_SCR_WRITE(P_temporal_OUT, 8 * padded, addr);
    SS_WAIT_SCR_WR();

    SS_REPEAT_PORT(n - 1);
    SS_RECURRENCE(P_temporal_invsqrt, P_temporal_DIV, 1);
    SS_SCR_PORT_STREAM(addr, 8, 8, n - 1, P_temporal_VEC);
    ...
    //SS_GARBAGE(P_temporal_O, (1 + total) * total / 2);
    //SS_WAIT_ALL();
    //return;
    ...
    SS_CONST(P_temporal_IN, 0, n * (n - 1) / 2);
    SS_GARBAGE(P_temporal_OUT, n * (n - 1) / 2);
  }
