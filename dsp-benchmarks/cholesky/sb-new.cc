...
void cholesky(complex<float> *a, complex<float> *L) {
  ...
  for (int i = 1; i < _N_; ++i) {
    ...
    SB_SCR_WRITE(P_multi_O, total * 8, 0);
    ...
    SB_RECURRENCE(P_multi_invpure, P_multi_V, 1);
    SB_WAIT_SCR_WR();
    SB_SCR_PORT_STREAM(0, 8, 8, total, P_multi_VEC);
    SB_DMA_WRITE(P_multi_fin, 8 * _N_, 8, total, L + (i + 1) * _N_ + i);
    SB_SCR_PORT_STREAM_STRETCH(0, 8, 8 * total, -8, total, P_multi_B);
    SB_CONFIG_PORT(total, -1);
    SB_SCR_PORT_STREAM(0, 8, 8, total, P_multi_A);
  }
  SB_WAIT_ALL();
}
