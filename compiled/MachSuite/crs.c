...
void spmv(TYPE *__restrict val, int64_t *__restrict n, int64_t *begin,
  ...
  #pragma ss config
  {
    arrayhint(val, M * sizeof(TYPE), 0);
    arrayhint(n, N * sizeof(TYPE), 0.75);
    arrayhint(begin, N * sizeof(TYPE), 0.75);
    arrayhint(vec, N * sizeof(TYPE), 1 - 1.0 / N);
    arrayhint(col, M * sizeof(TYPE), 0);
...
void run_accelerator(struct Arguments *args, int is_warmup) {
  if (is_warmup) {
    spmv(args->vals_, args->ns_, args->begin_, args->vec_, args->cols_, args->out_, args->total_);
  } else {
    spmv(args->vals, args->ns, args->begin, args->vec, args->cols, args->out, args->total);
  }
}
