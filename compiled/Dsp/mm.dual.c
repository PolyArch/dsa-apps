...
void mm(TYPE a[N * M], TYPE b[M * P], TYPE c[N * P]) {
#define PART 8
#define CHUNK (P / T / PART)
#define T 2
  #pragma ss config
  {
    arrayhint(a, N * sizeof(TYPE), 1 - (1.0 / CHUNK));
    arrayhint(b, N * sizeof(TYPE), 1 - 1.0 / P);
    arrayhint(c, N * sizeof(TYPE), 1 - (1.0 / (P / T)));
    TYPE bb[M * P];
    #pragma ss stream
