...
class DecisionTree {
  ...
  public:
    ...
    ITYPE data_val[N][M]; // load directly from your data
    ITYPE data_ind[N][M]; // indexes
	ITYPE labels[N][2];
    ...
    for(unsigned node_id=0; node_id<cur_nodes.size() && depth < max_depth; ++node_id){
	  ...
	  */
      ...
      for(int j=0; j<1; ++j) {
		...
		SB_DMA_READ(&data_val[0][0], 8, 8, data_size/4, P_sparse_none_feat_val);
		...
		// SB_DMA_READ(&common_inst_id[0], 8, 8, n, P_sparse_none_node_ind);
		// SB_DMA_READ(&node->inst_id[0], 8, 8, n, P_IND_DOUB0);
		// SB_DMA_READ(&common_inst_id[0], 8, 8, n, P_IND_DOUB0);
		// SB_DMA_READ(&x[0], 8, 8, n, P_IND_DOUB0);

        // SB_CONFIG_INDIRECT(T64, T64, 2);
        // SB_INDIRECT(P_IND_DOUB0, &labels[0][0], n*4, P_sparse_none_label);

        // SB_CONFIG_INDIRECT(T64, T64, 2);
        // SB_INDIRECT(P_IND_DOUB1, &labels[0][1], n*4, P_sparse_none_const);

        SB_CONST(P_sparse_none_local_offset, local_offset, n);


		// we don't know how many is required!: when all the addresses are
		// dummy, I want to reset: if it requires comparison, it looks like
		// adding that 0 is also not bad
