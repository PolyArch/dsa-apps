...
void build_histogram(long tid, struct TNode* node) {
  ...
  SB_ADD_PORT(P_IND_2);
  SB_ADD_PORT(P_IND_3);
  // SB_RECURRENCE(P_IND_4, P_IND_1, n/4);
  ...
  SB_CONFIG_INDIRECT1(T16, T64, 8*sizeof(uint16_t), 1);
  SB_INDIRECT_SCR(P_IND_1, getLinearAddr(0), n, P_gbdt_A);

  SB_CONFIG_INDIRECT(T16, T32, sizeof(uint32_t));
  SB_INDIRECT(P_IND_2, &hess[tid*NUM_THREADS], n, P_gbdt_hess);

  SB_CONFIG_INDIRECT(T16, T32, sizeof(uint32_t));
  SB_INDIRECT(P_IND_3, &grad[tid*NUM_THREADS], n, P_gbdt_grad);
...
void broadcast_from_cc(struct TNode* node) {
  ...
  n = (n/4)*4; // padding required because of indirect ports
  SB_DMA_READ(&node->inst_id[0], 8, 8, n/4, P_IND_1);
  SB_REM_PORT(P_IND_1, n/4, mask, P_IND_4);
  SB_WAIT_ALL();
}
