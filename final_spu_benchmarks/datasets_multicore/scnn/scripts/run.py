...
simulate.append("/home/vidushi/ss-stack/gem5/configs/example/se.py")
...
for n, m, s0, s1 in cases:
    ...
    subprocess.check_output('make cleandata', shell=True)
    subprocess.check_output('make %s' % env, shell=True)
    subprocess.check_output('make run', shell=True)
    raw = subprocess.check_output(simulate, shell=True).decode('utf-8')

    # TODO: extract the correct number of cycles  (max of all)
    cycles = None
