...
void* my_aligned_malloc(uint64_t align, uint64_t bytes)  {
  ...
  //FIXME: NOTE: TODO: Note that the softbrain simulator must touch each memory
  //inside the host core to bring it into OS.
  //so .. touch each page to bring into OS! -- yes this takes a long time
  //never mind that, touch each cache line to bring into l2
  for(unsigned i = 0; i < bytes; i+=32) {
    ptr[i]=0;
  }
  return (void*) ptr;
}
