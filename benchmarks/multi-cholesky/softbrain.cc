...
void cholesky(complex<float> *a, complex<float> *L) {
  ...
  complex<float> div;
  {
    ...
    union {
      float f[2];
      uint64_t v;
    } ri_norm = {-norm, -norm}, ri_v = {v.real(), v.imag()}, _;
    for (int j = 1; j < N; ++j) {
      int len = N - j;
    ...
    *L = sqrt(v);
    _.f[0] = _.f[1] = 1. / (L->real() * L->real() + L->imag() * L->imag());
    SB_CONTEXT(2);
    SB_DMA_READ(a + 1, 8, 8, N - 1, P_writeback_BP);
    SB_CONST(P_writeback_DIV, *((uint64_t *) L), N - 1);
    SB_CONST(P_writeback_NORM, _.v, N - 1);
