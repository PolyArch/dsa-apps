  ...
  */
  ...
  // begin_roi();
    for(int tile_no=0; tile_no<tile_factor; ++tile_no) { // tiling in neurons (TODO: check factor 8)
	  // FOR IN-ORDER CORE NUMBER OF Ti
...
int main() {
  ...
  synapse_val   = (VTYPE (*)[Ni][int(nnz1)])  malloc(int(nnz1)*Ni*sizeof(VTYPE));
  synapse_ind   = (VTYPE (*)[Ni][int(nnz1)])  malloc(int(nnz1)*Ni*sizeof(VTYPE));
