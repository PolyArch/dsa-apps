...
// dense versions
vector<uint16_t> dense_i_neuron[(Nx*Ny)/(Tx*Ty)][Ni];
vector<uint16_t> dense_synapse[Ni][Nn/Tn];
...
void load_weights_in_linear_scratch(int y, int x) {
  ...
  SB_DMA_SCRATCH_LOAD(&dense_synapse[y][x][0], 2, 2, size_synapse, getLinearAddr(getLinearOffset(0,2)));
...
void broadcast_weights(long tid, int y, int x) {
  ...
  unsigned size_synapse = dense_synapse[y][x].size();
...
void elate_weights() {
  for(int i=0; i<Ni; ++i) {
    for(int j=0; j<Nn/Tn; ++j) {
      for(int k=0; k<synapse_val[i][j].size(); ++k) {
        for(int n=0; n<synapse_ind[i][j][k]; ++n) {
          dense_synapse[i][j].push_back(0);
        }
        dense_synapse[i][j].push_back(synapse_val[i][j][k]);
      }
    }
  }
}

void elate_activations() {
  for(int i=0; i<(Nx*Ny/Tx*Ty); ++i) {
    for(int j=0; j<Ni; ++j) {
      for(int k=0; k<neuron_i_val[i][j].size(); ++k) {
        for(int n=0; n<neuron_i_ind[i][j][k]; ++n) {
          dense_i_neuron[i][j].push_back(0);
        }
        dense_i_neuron[i][j].push_back(neuron_i_val[i][j][k]);
      }
    }
  }
}
...
void fill_convolution_data() {
  ...
  // i.e. introduce 0s in it
  elate_weights();
  elate_activations();
  cout << "ELated activations and weights\n";
