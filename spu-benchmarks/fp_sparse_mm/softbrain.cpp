...
void mm_mult(std::pair<SBDT, double> *matrix1, int* row_ptr1, int nrows1, std::pair<SBDT, double> *matrix2, int* col_ptr2, int ncols2) {
  ...
  for (int i=0; i<nrows1; i++){
    ...
    for (int j=0; j<ncols2; j++){
      ...
      SB_DMA_READ(&(matrix2[ptr2].first), 8*2, 8, end2-ptr2, P_merge2way_B);
      SB_DMA_READ(&(matrix1[ptr1].second), 8*2, 8, end1-ptr1, P_merge2way_C);
      SB_DMA_READ(&(matrix2[ptr2].second), 8*2, 8, end2-ptr2, P_merge2way_D);
...
int main(int argc, char** argv){
   ...
   while(fgets(lineToRead, 500000, m1_file) != NULL) {
       ...
       if(!start){
           ...
           start = true;
            ...
            ++id2;
            sscanf(lineToRead, "%ld %d %lf", &t1, &row_id, &t2);
            row_id--;
            matrix1[id2] = std::make_pair((t1-1)%32768, t2);
    ...
    for(int j=nrows1-1; j>=0 && row_ptr1[j]==-1; j--){
      row_ptr1[j] = row_ptr1[j+1];
    }
  ...
  int *col_ptr2;
  std::pair<SBDT, double> *matrix2;
  nnz2 = nnz1;
  col_ptr2 = (int*)malloc((nrows1+1)*sizeof(int));
  matrix2 = (std::pair<SBDT,double>*)malloc(nnz1*sizeof(std::pair<SBDT,double>));
...
*/
  ...
  */
  ...
  mm_mult(matrix1, row_ptr1, nrows1, matrix2, col_ptr2, ncols2);
