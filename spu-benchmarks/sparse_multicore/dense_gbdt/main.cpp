...
void broadcast_node_id(struct TNode* node, unsigned n) {
  ...
  SS_INDIRECT(P_IND_2, &hess[0], n, P_gbdt_dum1);

  SS_CONFIG_INDIRECT(T64, T32, sizeof(uint32_t));
  SS_INDIRECT(P_IND_3, &grad[0], n, P_gbdt_dum2);

  SS_REM_PORT(P_IND_1, n, mask, P_IND_1);
  SS_REM_PORT(P_gbdt_dummy1, n, mask, P_gbdt_hess);
  SS_REM_PORT(P_gbdt_dummy2, n, mask, P_gbdt_grad);

  // SS_REM_PORT(P_IND_1, n, mask, P_IND_4);
  SS_WAIT_ALL();
  // SS_DMA_READ(&node->inst_id[0], 8, 8, n, P_IND_4);
  // SS_REM_PORT(P_IND_4, n, mask, P_IND_1);
...
void build_histogram(long tid, struct TNode* node) {
  ...
  SS_CONFIG_INDIRECT(T64, T64, M*sizeof(uint16_t));
  // SS_CONFIG_INDIRECT(T64, T64, 2);
  ...
  // SS_DMA_READ(&grad[0], 4, 4, n, P_gbdt_grad);
...
void build_histogram(long tid, struct TNode* node) {
  ...
  n = n/8;
  n = n*8;
  // cout << "NUMBER OF COMPUTATIONS: " << n <<endl;
  ...
  // port to port (ind4->ind1)
  // SS_ADD_PORT(P_IND_3);
  // SS_ADD_PORT(P_IND_2);
  // SS_RECURRENCE(P_IND_4, P_IND_1, n);

  // SS_ADD_PORT(P_IND_3);
  // SS_DMA_READ(&node->inst_id[0], 8, 8, n, P_IND_2);
  // Why is element left 936 here when M=16?
  // if(tid==0) { SS_WAIT_ALL(); return; }


  // FIXME: better maybe change the mapping
  // _index_addr + index * _ind_mult + _offsets[_index_in_offsets]*_data_bytes;
  SS_CONFIG_INDIRECT(T64, T64, M*sizeof(uint16_t)); // FIXME: this multiplier is good enough?
  SS_INDIRECT(P_IND_1, &fixed_inst_feat[0][j], n, P_gbdt_A);

  // this should be broadcast actually!
...
void broadcast1(struct TNode* node, unsigned n) {
  // cout << "COMES TO BROADCAST NODE\n";
  uint64_t mask=0;
  for(int i=0; i<NUM_THREADS; ++i) {
    addDest(mask, i);
  ...
  SS_CONFIG_INDIRECT(T64, T32, sizeof(uint32_t));
  SS_INDIRECT(P_IND_1, &grad[0], n, P_gbdt_dum2);
  SS_REM_PORT(P_gbdt_dummy2, n, mask, P_gbdt_grad);
...
void build_histogram(long tid, struct TNode* node) {
  ...
  // inst_id could be just 16-bit (this could be reduced, also it would need
  // padding then for n)
  if(tid==NUM_THREADS) { broadcast1(node, n); return;}
  ...
  // FIXME: CHECKME: I want it to be 16-bits -- should follow address datatype
  // SS_ATOMIC_SCR_OP(P_gbdt_C, P_gbdt_D, feature_offset, n*8*3, 0);
  // SS_ATOMIC_SCR_OP(P_gbdt_C, P_gbdt_D, feature_offset1, n*4*3, 0);
  // SS_ATOMIC_SCR_OP(P_gbdt_E, P_gbdt_F, feature_offset2, n*4*3, 0);
  SS_WAIT_SCR_WR();
  SS_WAIT_ALL();
}
*/
...
void *entry_point(void *threadid) {
  ...
  if(tid==NUM_THREADS) {
    ...
    cout << "TID not the same: " << tid << endl;
  // SS_CONFIG(gbdt_config, gbdt_size);
  // broadcast_node_id();
  build_histogram(tid, cur_node);
  */
...
void build_tree(struct TNode* node){
  ...
  for(unsigned node_id=0; node_id<cur_nodes.size() && depth < max_depth; ++node_id){
    ...
    assert(NUM_THREADS+1<C);

    // Barrier initialization
    // if(pthread_barrier_init(&barr, NULL, NUM_THREADS))
    if(pthread_barrier_init(&barr, NULL, NUM_THREADS+1))
    {
