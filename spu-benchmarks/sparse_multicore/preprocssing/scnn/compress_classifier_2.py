...
def get_activations():
    ...
    # Create the model
    model = create_model(args.pretrained, args.dataset, args.arch, parallel=not args.load_serialized, device_ids=args.gpus)
   
    checkpoint = torch.load('logs/2018.11.17-235159/checkpoint.pth.tar')
    ...
    # transforms.Scale(256) in place of resize
    ...
    img = Image.open("/home/vidushi/imagenet/test/cat.jpg")
    img_tensor = preprocess(img)
    img_tensor.unsqueeze_(0)
    print(type(model))
    print(model) # to print model info

    # input of classifier layer 0
    # model.classifier[0].register_forward_hook(printnorm)
    # model.classifier[1].register_forward_hook(printnorm)
    model.features.module[1].register_forward_hook(printnorm)

    # this is still final output
...
if __name__ == '__main__':
    ...
    # layer = model._modules.get('Conv2d')
    # print(layer)
    # h = layer.register_forward_hook(Variable(img_tensor))
    # # imm_output = model.features[:3](Variable(img_tensor))
    # # print(type(imm_output))
    # print(type(output))
    # # print(output)
