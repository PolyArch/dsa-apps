...
struct node** levelOrder(struct node** circuit, int index){
  ...
  result_circuit = (struct node**)malloc(sizeof(struct node*) * index);
								...
								stack<int> &Stack)
{
	...
	int ind;
	for (int i = 0; i < 2; ++i){
        ind = circuit[v]->child[i];
		if (!visited[ind]){
			topologicalSortUtil(ind, visited, Stack);
        }
    }
...
int main(int argc, char** argv) {
  ...
  // struct node **circuit_level_order = levelOrder(circuit, index);
  //levelOrder(circuit, index);
  topologicalSort(index+1);
