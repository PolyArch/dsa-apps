  ...
  for(int i=part_size; i<V; i+=part_size){
	...
	for(int j=i; j<(i+part_size) && j<V; j++){
	  ...
	  if(temp.c0>-1 && temp.c0 < i){
	    ...
	    x = part_size*(part_id-1) + cumsum(part_id-1) + shadow_nodes[part_id].size()-1; // shadow nodes would be put before this
	  } else { // if in the same partition
		x = temp.c0 + part_size*(part_id-1) + cumsum(part_id-1);
	  }
	  ac[j].c0 = x;
	  ...
	  if(temp.c1>-1 && temp.c1 < i){
		...
		x = temp.c1 + part_size*(part_id-1) + cumsum(part_id-1);
	  }
	  ac[j].c1 = x;
	}
  }
...
void store_ac_in_file() {
  ...
  string str1(dataset);
  char path1[100] = "datasets/";
  char x1[100] = "/final_circuit.data";
  
  ofstream ac_file (strcat(strcat(path1,str1.c_str()),x1)); // to store the nodes in top_order
  ...
  string str2(dataset);
  char path2[100] = "datasets/";
  char x2[100] = "/final_index.data";
  
  ofstream height_index (strcat(strcat(path2,str2.c_str()),x2));
  ...
  if(height_index.is_open()) {
    for(unsigned i=0; i<final_hgt_ptr.size(); i++) {
      // cout << final_hgt_ptr[i] << " ";
	  height_index << (final_hgt_ptr[i]-1) << "\n";
    }
  }
  height_index.close();
  ...
  string str3(dataset);
  char path3[100] = "datasets/";
  char x3[100] = "/final_shadow_index.data";
  
  ofstream shadow_index (strcat(strcat(path3,str3.c_str()),x3));
  ...
  if(shadow_index.is_open()) {
    for(unsigned i=0; i<shadow_hgt_ptr.size(); i++) {
	  shadow_index << (shadow_hgt_ptr[i]-1) << "\n";
	  if(i%2==1){
